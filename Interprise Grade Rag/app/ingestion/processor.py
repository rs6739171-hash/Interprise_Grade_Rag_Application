...
from app.services.retrieval.embeddings import embed_texts, embed_query, get_embedding_dim
...
logfire.configure(service_name="enterprise_ingestion_service", send_to_logfire="if-token-present")
#Local folder where paersed + chuncked Json metadata is saved 
PROCESSED_DATA_DIR = "processed_data"

# Initialeze Qdrant Client
...
def save_processed_locally (data:dict,source_type:str,filename:str)->str:
    """
    Save processed chunked data to a local JSON file with metadata.
    """
    ...
    with logfire.span("Processing File",file = filename,source = source_type):
        try:
            ...
            if ext == "pdf":
                ...
                logfire.warning(f"Skipping unsupported file type: {ext}", file=filename)
                return

            # Chunk the extracted text
            chunks = chunk_text(full_text)
            logfire.info("Chunking complete",file=filename,chunk_count = len(chunks))
            if not chunks:
                return
